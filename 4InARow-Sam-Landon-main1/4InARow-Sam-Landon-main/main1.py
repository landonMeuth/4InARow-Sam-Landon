      ...
      for column in range(len(board[row])):
        for scanDirection in [
          ...
          for i in range(len(word)):
            if (row + (i * scanDirection[0])) < 0 or (
                row + (i * scanDirection[0])) > (len(board) - 1) or (
                  column + (i * scanDirection[1])) < 0 or (
                    column + (i * scanDirection[1])) > (
                      len(board[row]) -
                      1):  #makes sure the scanner doesn't go out of bounds
              pass
            elif word[i].lower() == board[row + (i * scanDirection[0])][
                column +
              (i *
               scanDirection[1])].lower():  #each letter is thouroughly checked
              checksum += 1
...
def makeAMove(x, y):
  ...
  while True:  #loop makes sure input is valid
    ...
    if move in [1, 2, 3, 4, 5, 6, 7]:
      if board[0][move - 1] in ["x",
                                "o"]:  #disallows putting peice in full column
        break
      else:
        while board[gravity][move - 1] in ["x",
                                           "o"]:  #simulates gravity of peice
          gravity -= 1
